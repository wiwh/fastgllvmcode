...
#'
#'
#' #' Computes the hessian matrix, which is block-diagonal for A, B, and diagonal for phi,
#' compute_hessian <- function(fg, H=1) {
#'   Z <- scale(gen_Z(fg$dimensions$n, fg$dimensions$q), scale=F, center=T)
#'   warning("Z has been rescaled in simulate_hessian_AB")
#'   linpar <- compute_linpar(Z, fg$parameters$A, fg$X, fg$parameters$B)
#'   linpar_bprimeprime <- compute_linpar_bprimeprime(linpar$linpar, fg$families)
#'   ZX <- cbind(Z, fg$X)
#'   bdiag_AB <- bdiag_m(sapply(1:fg$dimensions$p, function(j) {
#'     -solve((t(ZX) %*% (ZX*(linpar_bprimeprime[,j])))/(fg$parameters$phi[j]*nrow(ZX)))
#'   }, simplify=F))
#'   bdiag_phi_covZ <- -Matrix::Diagonal(fg$dimensions$p + fg$dimensions$q**2)
#'   Matrix::bdiag(bdiag_AB, bdiag_phi_covZ)
#' }
#'
