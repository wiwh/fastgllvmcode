...
ff_learning_rate <- function(method="constant", learning_rate.args=list(start=40, end=0.1)){
  if(method=="constant"){
    if(is.null(learning_rate.args$constant))stop("Missing learning_rate.args$constant. Provide a value for the constant.")
    learning_rate <- function(i){
      learning_rate.args$constant
    }
  }
  if(method=="lin"){
  ...
  if(method=="exp"){
    stopifnot(learning_rate.args$end>0)
    lr <- with(learning_rate.args, {
  ...
  if(method=="spall"){
    if(is.null(learning_rate.args$a)) learning_rate.args$start <- 40
    if(is.null(learning_rate.args$b)) learning_rate.args$end <- 0.1
