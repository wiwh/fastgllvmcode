...
if(0) {
  ...
  fg <- gen_fastgllvm(nobs=1000, p=p, q=q, family=c(rep("poisson", poisson), rep("gaussian", gaussian), rep("binomial", binomial)),k=0, intercept=T, miss.prob = 0, scale=1)
  ...
  for(i in 1:100){
    ...
    if(i < 10) {
      values <- update(fg$Y, fg$X, values$parameters, values$gradients, fg$families, fg$Miss, alpha=2/(2+i**.2), beta=0.8, debiase=F, compute_gradients = compute_gradients)
    } else {
      values <- update(fg$Y, fg$X, values$parameters, values$gradients, fg$families, fg$Miss, alpha=2/(2+i**.2), beta=0.8, debiase=T,  compute_gradients = compute_gradients)
    }
  ...
  plot(fg$parameters$B, values$parameters$B)
  points(fg$parameters$B, t(fit.gmf$beta))
