...
# Computes the procrustes error between two matrices of loadings

procrustes <- function(L, target){
  Lrot <- psych::Procrustes(L, target)$loadings
  Error <- norm(Lrot - target, "f")
  list(Lrot=Lrot, Error=Error)
}
