...
* $\beta_j(\eta_{ij}) = \eta_{ij} - \text{sig}(\eta_{ij})$, whith $\text{sig}(x) = 1/(1+\exp(-x))$
* $\beta_j'(\eta_{ij}) = 1 - \frac{\text{sig}'(\eta_{ij})}{\text{sig}(\eta_{ij})} = \text{sig}(\eta_{ij})$
* $c_j(y_{ij}, \tau_j) = 0$
* $\tau_j=1$
...
dlogfYZ <- function(Y, Z, par){
  n <- nrow(Y)
  natpar <- Z %*% t(par$A)
  deriv <- t(sapply(1:n, function(i){
    as.vector(t((Y[i,] - sigmoid(natpar)[i,]) %*% t(Z[i,])))
  }))
  deriv
}

# same as dlogfYZ but for a single observation
dlogfYZi <- function(Yi, Zi, par){
  ...
  A.deriv <- 
    t(sapply(1:n, function(i){
      rowMeans(sapply(1:nN, function(iN){
        dlogfYZi(Y[i,], Z[iN, ], par) * fYZ(Y[i,,drop=F], Z[iN,,drop=F], par)
      }))
    }))
  A.deriv
}
...
as a sanity check, the expectation of the scores must be 0 at the true value... let's check this
...
We need its derivative w.r.t. $\theta$, which quickly becomes a mess. Instead, we use Spall's method.
...
H <- compute_expected_hessian(generator=generator_MLE, score=score_MLE, theta=theta, N=N, c=0.01)$H
```
