...
compute_scores_Y <- function(Y, par, nD, nN, outcomes_scores=NULL){
  ...
  if(is.null(outcomes_scores)){
    outcomes <- compute_outcomes(p)
    outcomes_scores <- compute_scores(outcomes, par, nD, nN)
  }
...
par$A <- matrix(c(-2, -2, -1, -1, 1, 1, 2, 2), p, 1)
...
score_to_derive <- function(Y, theta, par){
  p <- nrow(par$A)
  q <- ncol(par$A)
  par2 <- par
...
$$
\text{ARE} = \frac{\text{trace}\left(A(\theta)^{-1}B(\theta)\left[A(\theta)^{-1}\right]^\top\right)}{\text{trace}(I^{-1})}
$$

```{r}
ARE = sum(diag(Avar))/sum(diag(Ineg))
```
