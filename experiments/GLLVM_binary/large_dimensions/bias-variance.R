...
loadings <- sapply(1:nrow(settings), function(i) {
  ...
  loadings <- lapply(methods, function(method){
    ...
    tibble(mpe=sqrt(mse), bias=bias, true=ma(loadings$true[1,], 5)) %>% mutate(n=settings[i,1], p=settings[i,2], setting=settings[i,3], Estimator=method) %>%
...
pA <- loadings %>% filter(setting=="Setting A", p %in% c(100, 500, 1000)) %>%
