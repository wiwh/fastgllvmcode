...
est.sprime <- function(dat){
  tm <- proc.time()
  fit.sa <- fastgllvm(dat$Y, dat$dimensions$q, family = "poisson", intercept = T, method="simple", alpha=.1, maxit=100)
  tmdiff <- proc.time()-tm
  fit.sa <- update(fit.sa)
...
est.prime <- function(dat){
  tm <- proc.time()
  fit.sa <- fastgllvm(dat$Y, dat$dimensions$q, family = "poisson", intercept = T, method="full", alpha=.1, maxit=100)
  tmdiff <- proc.time()-tm
...
if(0){
  ...
  fit.fg <- fastgllvm(fg$Y, q=q, family="poisson", intercept=T, alpha=.2, maxit = 50, method="simple")
  plot(fit.fg)
  fit.fg <- update(fit.fg)
  plot(fit.fg)


  library(gmf)
  fit.gmf <- gmf(fg$Y, p=q, intercept=T, family=poisson(), method="quasi")
