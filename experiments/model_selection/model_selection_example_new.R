...
modelselec <- function(Y, X, M=NULL, fit.init=NULL, family="gaussian", get_criterion=get_criterion_factory(Y, X, method="BIC", family=family), mod.gen=100, mod.keep=20, init=NULL, iter=100, prop.active.init=0.2, unique=T){
  p <- ncol(X)
  fit <- fit.init
...
# # testing fastgllvm here...
# probs <- 1/(1+exp(-(-3 + 100*abs(beta0) + rnorm(length(beta0), 0, 0.5))))
# M <- t(sapply(1:1000, function(na)rbinom(length(beta0), size=1, prob=probs)))
# M <- selec$Best
#
#
#
#
# fit <- fastgllvm_oop(M, q=1, X=1, maxit=1000, tol=0, family=binomial())
# plot(log(probs/(1-probs)), fit$B); abline(0,1,col=2)
# Msim <- simulate_fastgllvm(fit, 10000)$Y
# plot(colMeans(M), colMeans(Msim))


selec <- modelselec(Y0, X0, M=NULL, fit.init=NULL, mod.gen = 100, mod.keep=50, prop.active.init = .5, iter=20, family=family, unique=unique)

selec <- modelselec(Y0, X0, M=selec$Best,  fit.init=selec$fit, mod.gen = 100, mod.keep=50, prop.active.init = .5, iter=20, family=family, unique=unique)
...
# TODO: the model selection criterion should not be based on ALL of the datasets... only on some part of it, if we want to detect covariances. For instance, instead of overall
# classification error sum_i^n l(yhat_i, y_i), take the best 80%, say. (i.e., remove the 20% worst performing). or select many and accept them as a group ... ?
# or maybe regress the models on the values of the covariates? ouchouchouch
