...
get_criterion_factory <- function(Y,X, method="BIC", family="gaussian", prop.data=1){
  ...
  if(method=="BIC"){
    if(prop.data==1){
      # compute the criterion on the whole dataset
      ...
      get_criterion <- function(models){
        #compute the criterion on a subset of data to capture other structures
        s <- sample(length(Y), length(Y)*prop.data, repl=T)  ## TODO: maybe replace=F
        crit <- apply(models, 1, function(model) BIC(glm(Y[s]~0 + X[s, model,drop=F], family=family)))
        list(crit=crit)
      }
    }
  }
...
s <- sample(n, n/4)
gn <- glmnet(x=X0[s,], y=Y0[s], family=family)
View(as.matrix(gn$beta))
ntree <- 500
rf <- randomForest(x=X0, y=as.factor(Y0), ntree=ntree, localImp=T, mtry=20)
image(cov(t(rf$localImportance)))

# get the split variables
splitvars <- t(sapply(1:ntree, function(k) (1:p) %in%unique(getTree(rf, k)[,"split var"])[1:4]))
image(cov(splitvars))

fg <- fastgllvm_oop(splitvars, X=1)
plot_fastgllvm(fg)
...
M.fake <- NULL
for(i in 1:100){
  M.fake <- rbind(M.fake, trueModel01)
  M.fake <- rbind(M.fake, trueModel02)
}

fit <- fastgllvm(M.fake, q=1, X=1)


get_criterion <- get_criterion_factory(Y0[1:(n/2)], X0[(n/2+1):n, ], method="BIC", family=family, prop.data = .5)
get_criterion(rbind(trueModel01, trueModel02))

get_criterion <- get_criterion_factory(Y0, X0, method="BIC", family=family, prop.data = .05)
get_criterion(rbind(trueModel01, trueModel02))


BIC.true <- t(sapply(1:500, function(na){
  set.seed(na)
  M.true <- rbind(trueModel01, trueModel02)

  M.random <- t(sapply(1:5, function(na){
    s <- sample(p, 8, replace=F)
    ((1:p) %in% s + t(beta02!=0)) > 0
  }))
  # M.random <- NULL
  M <- rbind(M.true, M.random)
  get_criterion(M)$crit
}))

boxplot(BIC.true, outline=F)

selec <- modelselec(Y0, X0, M=NULL, fit.init=NULL, mod.gen = 100, mod.keep=50, prop.active.init = .5, iter=100, family=family, unique=unique, prop.data = .05)


# we can continue the fit here...
# selec <- modelselec(Y0, X0, M=selec$Best,  fit.init=selec$fit, mod.gen = 100, mod.keep=50, prop.active.init = .5, iter=200, family=family, unique=unique, prop.data = .5)
